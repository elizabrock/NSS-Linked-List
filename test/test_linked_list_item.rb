...
class LinkedListItemTest < Test::Unit::TestCase
  ...
  def test_39a_comparator_greater_than
    lli1 = LinkedListItem.new("foo")
    lli2 = LinkedListItem.new("bar")
    assert lli1 > lli2
  end

  def test_39b_comparator_equal_to
    lli1 = LinkedListItem.new("foo")
    lli2 = LinkedListItem.new("foo")
    assert lli1 == lli2
  end

  def test_39c_comparator_lesser_than
    lli1 = LinkedListItem.new("foo")
    lli2 = LinkedListItem.new("bar")
    assert lli2 < lli1
  end

  def test_39d_comparator_symbol
    lli1 = LinkedListItem.new(:foo)
    lli2 = LinkedListItem.new("bar")
    assert lli1 > lli2
    assert lli2 < lli1
  ...
  def test_39e_comparator_integer_string
